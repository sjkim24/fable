class Api::StoriesController < ApplicationController
  def index
    if params[:tag]
      tag_id = Tag.find_by_tag_desc(params[:tag].downcase)
      if tag_id
        @stories = Story.joins(:taggings).where("taggings.tag_id = ?", tag_id)
      else
        @stories = []
      end
    else
  ...
  def get_comments_only
    story = Story.find(params[:id])
    @comments = story.comments.where(parent_comment_id: nil)
